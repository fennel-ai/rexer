...
class Client(object):
    ...
    def query(self, query: Expr):
        ...
        if not isinstance(query, Expr):
            raise InvalidInput(
                "query expected to be an RQL Expr but got '%s' instead" % query
            )
    ...
    def set_profile(self, item: profile.ProfileItem):
        if not isinstance(item, profile.ProfileItem):
            raise InvalidInput(
                "arg of set profile should be ProfileItem but got: %s" % item
            )
    ...
    def get_profile(self, item: profile.ProfileItem):
        if not isinstance(item, profile.ProfileItem):
            raise InvalidInput(
                "arg of get profile should be ProfileItem but got: %s" % item
            )
        errors = profile.validate(item)
        if len(errors) > 0:
            raise InvalidInput("invalid profile item: %s" % ", ".join(errors))
    ...
    def get_profiles(self, pfr: profile.ProfileFetchRequest):
        if not isinstance(pfr, profile.ProfileFetchRequest):
            raise InvalidInput(
                "fetch arg not a ProfileFetchRequest object: %s" % str(pfr)
            )
    ...
    def fetch(self, afr: action.ActionFetchRequest):
        if not isinstance(afr, action.ActionFetchRequest):
            raise InvalidInput(
                "fetch arg not an ActionFetchRequest object: %s" % str(afr)
            )
    ...
    def aggregate_value(self, request: aggregate.GetAggValueRequest):
        if not isinstance(request, aggregate.GetAggValueRequest):
            raise InvalidInput(
                "arg to aggregate_value must be GetAggValueRequest but instead got: %s"
                % str(request)
            )
    ...
    def store_aggregate(
        self, agg_type: str, agg_name: str, query: Expr, options: aggregate.AggOptions
    ):
