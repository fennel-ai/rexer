...
func TestAirbyteSourceClient(t *testing.T) {
	client, err := NewClient("http://localhost:8002/api", "airbyte_topic")
	assert.NoError(t, err)
	fmt.Println(client.getSourceDefinitionId("S3"))
	s3Source := data_integration.S3{}
	s3Source.Name = "unit_test_s3_try5"
	s3Source.Bucket = "aditya-temp"
	s3Source.PathPrefix = "movie_lens"
	s3Source.AWSAccessKeyId = "AKIAQOLFGTNXJY2MZWLP"
	s3Source.AWSSecretAccessKey = "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"
	s3Source.Format = "csv"
	sid, err := client.CreateSource(s3Source)
	assert.NoError(t, err)
	fmt.Println(sid)
	...
	`
	sid, err = client.CreateSource(bigQuerySource)
	fmt.Println(sid)
	assert.NoError(t, err)
}
