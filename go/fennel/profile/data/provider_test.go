...
func testProviderBasic(t *testing.T, p Provider) {
	...
	// now set the value
	err = p.Set(1, 1232, "summary", 1, expected)
	assert.NoError(t, err)
...
func testProviderVersion(t *testing.T, p Provider) {
	...
	err = p.Set(1, 1232, "summary", 10, expected3)
	assert.NoError(t, err)
