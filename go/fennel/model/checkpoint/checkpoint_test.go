...
func TestCheckpoint2(t *testing.T) {
	...
	checkpoint, err := Get(tier, aggtype, aggname)
	assert.NoError(t, err)
	assert.Equal(t, zero, checkpoint)

	// now set a checkpoint
	expected1 := ftypes.OidType(1)
	err = Set(tier, aggtype, aggname, expected1)
	assert.NoError(t, err)
	...
	err = Set(tier, aggtype, aggname, expected2)
	assert.NoError(t, err)
	checkpoint, err = Get(tier, aggtype, aggname)
	assert.NoError(t, err)
	...
	// initially no checkpoint is setup, so we should get 0
	checkpoint, err = Get(tier, aggtype, aggname2)
	assert.NoError(t, err)
	assert.Equal(t, zero, checkpoint)

	expected3 := ftypes.OidType(51)
	err = Set(tier, aggtype, aggname2, expected3)
	assert.NoError(t, err)
