...
func testProviderBasic(t *testing.T, p provider) {
	...
	// now set the value
	err = p.set(this, 1, 1, 1232, "summary", 1, expected)
	assert.NoError(t, err)
...
func testProviderVersion(t *testing.T, p provider) {
	...
	err = p.set(this, 1, 1, 1232, "summary", 10, expected3)
	assert.NoError(t, err)
	...
	found, err = p.get(this, 1, 1, 1232, "summary", 2)
	assert.NoError(t, err)
	assert.Equal(t, expected2, found)

	found, err = p.get(this, 1, 1, 1232, "summary", 10)
	assert.NoError(t, err)
