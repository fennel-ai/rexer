...
func (s knn) Transform(v value.Value) (value.Value, error) {
...
func (s knn) extract(v value.Value) (value.List, error) {
	l, ok := v.(value.List)
	if !ok {
...
func (s knn) Reduce(values []value.Value) (value.Value, error) {
	z := s.Zero().(value.List)
	for i := range values {
...
func (s knn) Merge(a, b value.Value) (value.Value, error) {
	la, err := s.extract(a)
	if err != nil {
		return nil, err
	}
