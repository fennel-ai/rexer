...
func TestExtractor_Apply(t *testing.T) {
	...
	scenarios := []struct {
	...
	}{
		{
			...
			false,
			[]value.Value{value.Int(10), value.Int(17), value.Int(10), value.Int(3), value.String("2022-06-16T01:36:44+08:00")},
		},
		{
			[]value.Value{
			...
			[]value.Dict{
				value.NewDict(map[string]value.Value{"from": value.Int(1655055404), "part": value.String("dayofweek")}),
				value.NewDict(map[string]value.Value{"from": value.Int(1655055404 + 86400), "part": value.String("dayofweek")}),
				value.NewDict(map[string]value.Value{"from": value.String("2022-06-16T01:36:44+08:00"), "part": value.String("timestamp")}),
				value.NewDict(map[string]value.Value{"from": value.String("2022-06-16T01:36:44+08:00"), "part": value.String("rfc3339"), "timezone": value.String("America/Los Angeles")}),
			},
			false,
			[]value.Value{
				...
				value.NewDict(map[string]value.Value{"myfield": value.String("2022-06-16T01:36:44+08:00"), "outputfield": value.String("2022-06-16T10:36:44+-07:00")}),
			},
		},
	}
