...
func TestFilterOperator_Apply(t *testing.T) {
	...
	optest.Assert(t, tier, &FilterOperator{}, whereTrue, intable, contextKwargTable, expected)

	// and when we filter everything, we should get empty table
	contextKwargTable = []value.Dict{whereFalse, whereFalse, whereFalse}
	optest.Assert(t, tier, &FilterOperator{}, whereTrue, intable, contextKwargTable, []value.Dict{})
}
...
func TestTakeOperator_Apply(t *testing.T) {
	...
	optest.Assert(t, tier, &TakeOperator{}, value.Dict{"limit": value.Int(2)}, intable, contextKwargTable, expected)

	// and when the limit is very large, it only returns intable as it is
	optest.Assert(t, tier, &TakeOperator{}, value.Dict{"limit": value.Int(10000)}, intable, contextKwargTable, intable)
}
...
func TestExplodeOperator_KeyNotString(t *testing.T) {
	...
	optest.AssertError(t, tier, &ExplodeOperator{}, skwargs, intable, []value.Dict{{}})
...
func TestExplodeOperator_ScalarKeys(t *testing.T) {
	...
	optest.Assert(t, tier, &ExplodeOperator{}, skwargs, intable, []value.Dict{{}}, intable)
}
...
func TestExplodeOperator_Apply(t *testing.T) {
	...
	optest.Assert(t, tier, &ExplodeOperator{}, skwargs, intable, contextKwargTable, expected)
