...
func TestInterpreter_VisitList(t *testing.T) {
	// Empty list works
	testValid(t, ast.MakeList(), value.NewList())
...
func TestInterpreter_VisitDict(t *testing.T) {
	// Empty dict works
	testValid(t, ast.MakeDict(nil), value.NewDict(nil))

	// dict with just one element works
	d := value.NewDict(map[string]value.Value{"hi": value.Double(3.4)})
	testValid(t, ast.MakeDict(map[string]ast.Ast{"hi": ast.MakeDouble(3.4)}), d)
	...
	testValid(t, ast.MakeDict(map[string]ast.Ast{
		"hi":  ast.MakeDouble(3.4),
		"bye": ast.MakeBool(false),
		"nested": ast.MakeDict(map[string]ast.Ast{
			"hi": ast.MakeDouble(3.4),
		}),
	}), nested)
}
...
func TestInterpreter_VisitLookup(t *testing.T) {
	...
	testError(t, &ast.Lookup{On: ast.MakeDict(nil), Property: "hi"})

	// dict with just one element works only if property is same
	d := ast.MakeDict(map[string]ast.Ast{"hi": ast.MakeDouble(3.4)})
...
func TestFailEmptyOperand(t *testing.T) {
	i := getInterpreter(nil, value.Dict{})
	_, err := i.VisitOpcall([]ast.Ast{}, []string{}, "std", "set", ast.MakeDict(nil))
	require.Error(t, err)
}
...
func TestVisitOpCallMultiop(t *testing.T) {
	...
	q := ast.OpCall{
		Operands: []ast.Ast{
			...
			ast.MakeList(ast.MakeInt(3), ast.MakeInt(4)),
		},
		Vars:      []string{"a", "b"},
		Namespace: "std",
		Name:      "map",
...
func TestInterpreter_VisitVarClone(t *testing.T) {
	...
	query := ast.Query{Statements: []*ast.Statement{
		{
			Name: "x",
			Body: ast.MakeDict(map[string]ast.Ast{"key": ast.MakeInt(0)}),
		},
		{
			Name: "y",
			Body: &ast.OpCall{
				Namespace: "std",
				Name:      "set",
				Operands:  []ast.Ast{ast.MakeList(&ast.Var{Name: "x"})},
				Vars:      nil,
				Kwargs: ast.MakeDict(map[string]ast.Ast{
		...
		{
			Name: "",
			Body: ast.MakeDict(map[string]ast.Ast{
				"x": &ast.Var{Name: "x"},
				"y": &ast.Var{Name: "y"},
			}),
		},
