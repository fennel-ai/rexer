...
func TestActionDBBasic(t *testing.T) {
	...
	found, err := table.actionDBGet(request)
	assert.NoError(t, err)
	assert.Empty(t, found)
	// let's add an action
	action1 := action.Action{ActorID: 111, ActorType: 11, TargetType: 12, TargetID: 121, ActionType: 13, ActionValue: 14, Timestamp: 15, RequestID: 16}
	aid1, err := table.actionDBInsert(action1)
	assert.NoError(t, err)

	action2 := action.Action{ActorID: 211, ActorType: 21, TargetType: 22, TargetID: 221, ActionType: 23, ActionValue: 24, Timestamp: 25, RequestID: 26}
	aid2, err := table.actionDBInsert(action2)
	assert.NoError(t, err)
