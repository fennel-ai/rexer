...
func NewTailer(n nitrous.Nitrous, topic string, toppars kafka.TopicPartition, store hangar.Hangar, processor EventsProcessor,
		...
		RebalanceCb: mo.Some(func(c *kafka.Consumer, e kafka.Event) error {
			...
			case kafka.AssignedPartitions:
				if len(event.Partitions) > 0 {
					...
					var err error
					toppars, err := decodeOffsets(kafka.TopicPartitions{toppars}, store)
					if err != nil {
