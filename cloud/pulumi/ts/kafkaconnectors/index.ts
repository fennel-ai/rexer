...
export const setup = async (input: inputType): Promise<pulumi.Output<outputType>> => {
    ...
    // TODO(mohit): Deprecate confluent connectors once MSK connectors are running
    ...
    // MSK connector
    const awsProvider = new aws.Provider("training-data-msk-connect-provider", {
    ...
    const logGroup = new aws.cloudwatch.LogGroup("training-data-connector-log-group", {
        name: `/aws/msk/t-${input.tierId}-training-data-connector`,
        // retain logs for 5 days
        retentionInDays: 5,
    }, { provider: awsProvider });

    // Create IAM role which has access to the S3 bucket
    const trainingDataConnectorRole = new aws.iam.Role("training-data-connector-role", {
        namePrefix: `t-${input.tierId}-training-data-connector`,
        description: `IAM role for Training Data MSK Connector for tier - ${input.tierId}.`,
        assumeRolePolicy: {
            "Version": "2012-10-17",
            "Statement": [
                {
        ...
        inlinePolicies: [{
            name: `t-${input.tierId}-training-data-connector-policy`,
            policy: `{
                "Version":"2012-10-17",
                "Statement":[
                    {
            ...
            name: `t-${input.tierId}-training-data-connector-msk-access`,
    ...
    new aws.mskconnect.Connector("training-data-msk-connector", {
        name: `t-${input.tierId}-training-data-connector`,
        description: `MSK Connector to generate Training Data in cold storage for tier ${input.tierId}`,
        ...
        serviceExecutionRoleArn: trainingDataConnectorRole.arn,
        plugins: [{
            customPlugin: {
