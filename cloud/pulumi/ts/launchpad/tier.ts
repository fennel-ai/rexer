...
const setupResources = async () => {
    ...
    const rng = seedrandom(`t-${input.planeId}`);
    ...
    const configsOutput = pulumi.all([input.dbPassword, input.kafkaApiSecret]).apply(async ([dbPassword, kafkaPassword]) => {
        return await configs.setup({
            ...
            modelServingConfig: pulumi.output({
                "region": input.region,
                "executionRole": sagemakerOutput.roleArn,
                "privateSubnets": sagemakerOutput.subnetIds.apply(subnetIds =>  { return subnetIds.join(","); }),
